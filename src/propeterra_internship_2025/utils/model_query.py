...
class QueryModel():
    ...
    def __init__(self, model:str, country:str, prompt:str, prompt_number:int):
        self.model = model
        ...
        self.SUPPORTED_MODELS = {"gpt-4.1":os.environ.get("OPENAI_API_KEY"),
                                 "sonar_pro":os.environ.get("PERPLEXITY_API_KEY_PROPETERRA"),
    ...
    def write_to_file(self, model:str, country:str , response, prompt_number:int):
        ...
        with open(f"model_output/{model}_{country}_prompt_{prompt_number}.txt", "a", encoding="utf8") as file:
            file.write(response)
        print(response)
...
ai_num_links:
                    ...
                    '''
        with open(f"./model_output/{self.model}_{self.country}_{date.today()}_prompt_{self.prompt_number}.txt", "a", encoding="utf8") as file:
            file.write(template)
        # print(template)
    ...
    def query_sonar_pro(self) -> None:
        ...
        self.write_to_file(self.model, self.country, self.system_prompt, self.prompt_number)
